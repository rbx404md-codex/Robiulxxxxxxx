...
def create_checker_menu_keyboard():
    ...
    keyboard = [
        [
            InlineKeyboardButton("🔍 Single AUTH", callback_data="check_auth"),
            InlineKeyboardButton("💰 Single CHARGE", callback_data="check_charge")
        ],
        [
            InlineKeyboardButton("📋 Mass AUTH", callback_data="check_mauth"),
            InlineKeyboardButton("💎 Mass CHARGE", callback_data="check_mcharge")
        ],
        [
...
TELEGRAM_BOT_TOKEN = '8681447507:AAHdKtszBnWCM3rRk12BxbJIUPY3xGz1Mgw'
TOKEN = '8681447507:AAHdKtszBnWCM3rRk12BxbJIUPY3xGz1Mgw'
#-------×COMMAND_HANDLER_TOKEN×----------#
...
OWNER_ID = 7294948308  # configured owner id
...
"""
        ...
        elif data == "check_charge":
            text = """
💰 **SINGLE CHARGE CHECK**
━━━━━━━━━━━━━━━━━━━━

**Usage:**
`/charge 4532xxxxxxxx|12|2025|123`
...
async def process_card_list(update: Update, context: ContextTypes.DEFAULT_TYPE, cards: list, username: str):
    ...
    deduct_credits(user_id, total_cards)
    ...
    job = create_job(
        ...
        gate=None,  # optional gate
        get_daily_count=get_daily_count,
        increment_daily=increment_daily,
        on_result=send_card_result_callback  # we'll define below
    )
...
async def generate_gift(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not (is_owner_uid(user_id) or is_admin_uid(user_id)):
        await update.message.reply_text("Only Owner and Admins are allowed to use this command")
        return

    if is_admin_uid(user.id) and not is_owner_uid(user.id) and amount > 200:
...
SESSION_STRING = "BQGZ-I0AGj2vX0J3GCqnR8J-l7IUtu25qeAkr6KQezLpNdsrmlcqwubEaIutASMAu0OdpMHV0TtYjZ2OUDEiLw_oSdgUVWKVJd8ffgcF-6qCMFMHbLv2znq64H6eeBahMqcq9RyoQuqSOt5G0GYOoVndD63ZdUpwE9C-BlR_HqbohhUtcW82ilw9k2s2v_7F-WfsDeGJHMZznfoqGSz2yeG9qVWpURMxi4rPi86grvADXTNLRTx0cs-YWejsL-uIMngPX1VXg_nO6acvryKTcd6cT1VFbsAuUJzvPVs-e-WTP1-fo6-NDfyfwr8vTn7YeVji3boGF3j26k8IYEgLunKrjXyqIAAAAAFtPHjwAA"
...
# ========== END ENHANCED CHECKER COMMANDS ==========
    ...
    out = "\n".join(f"{r['card']} => {r['status']} | {r['message']}" for r in results)
...
# Register handlers — try to add gracefully if Application 'app' exists at import time
try:
    app  # noqa: F821
except Exception:
    app = None
